...
export function Tasker() {
  ...
  const [taskCount, setTaskCont] = useState<number>(0);

  const [taskCompleted, setTaskeCompleted] = useState<number>(0);
  ...
  return (
    <div className={style.tasker}>
      <div className={style.taskDiv}>
        ...
        >
          Criar{" "}
          <img src="./src/assets/plus.svg" className={style.plus} alt="plus" />
        </button>
      </div>
      <div className={style.tasker}>
        <div className={style.progress}>
          ...
          <div className={style.taskCounters}>
            ...
            <p
              className={style.taskCounter}
            >{`${taskCompleted} de ${taskCount}`}</p>
          </div>
        </div>
        <div className={style.taskList}>
          {tasks.map((tasks) => {
            return (
              <Task key={tasks} content={tasks} onDeleteTask={deleteTask} />
            );
          })}
        </div>
      </div>
